...
class SAMbasedAdaptationModule(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
        ...
        if isinstance(n_conv_per_stage_decoder, int):
            n_conv_per_stage_decoder = [n_conv_per_stage_decoder] * (n_stages - 1)
        #断言确保n_conv_per_stage的长度等于n_stages，即每个阶段都有对应的卷积层数
        assert len(n_conv_per_stage) == n_stages, "n_conv_per_stage must have as many entries as we have " \
                                                  f"resolution stages. here: {n_stages}. " \
                                                  f"n_conv_per_stage: {n_conv_per_stage}"
        #断言确保n_conv_per_stage_decoder的长度等于(n_stages - 1)，即解码器的每个阶段都有对应的卷积层数
        assert len(n_conv_per_stage_decoder) == (n_stages - 1), "n_conv_per_stage_decoder must have one less entries " \
                                                                f"as we have resolution stages. here: {n_stages} " \
                                                                f"stages, so it should have {n_stages - 1} entries. " \
                                                                f"n_conv_per_stage_decoder: {n_conv_per_stage_decoder}"
        #使用PlainConvEncoder类初始化编码器部分，该部分包含卷积层和规范化操作
        self.encoder = PlainConvEncoder(input_channels, n_stages, features_per_stage, conv_op, kernel_sizes, strides,
                                        n_conv_per_stage, conv_bias, norm_op, norm_op_kwargs, dropout_op,
                                        dropout_op_kwargs, nonlin, nonlin_kwargs, return_skips=True,
                                        nonlin_first=nonlin_first)
        #使用SAMDecoder类初始化解码器部分，该部分用于解码并生成输出。
        self.decoder = SAMDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision,
                                   nonlin_first=nonlin_first)
        self.deconv = nn.ConvTranspose2d(3, 3, 2, 2, output_padding=0)  
        ...
        #将图像编码器参数的梯度计算关闭，即冻结这些参数。
        for param in self.sam_image_encoder.parameters():
            param.requires_grad = False

    def forward(self, x):
        # 将输入x进行分离，得到sam_input，这一步是为了防止梯度回传到输入
        sam_input = x.detach()
        #如果通道数为1，则将其复制为3个通道，这是为了适应模型的输入要求。
        if sam_input.shape[1] == 1:
            sam_input = sam_input.repeat(1, 3, 1, 1)
        #print(f'the shape of the patch_input is: {sam_input.shape}')
        #对输入进行双线性插值，将其调整为指定大小。
        #sam_input = F.interpolate(sam_input, size=(1024, 1024), mode='bilinear', align_corners=True)
        #使用反卷积层对输入patch进行上采样，将其调整为指定大小
        sam_input = self.deconv(sam_input)
        #print(f'the shape of the patch_input after interpolate is: {sam_input.shape}')
        #将调整后的输入传递给sam编码器部分，得到特征表示sam_embed
        sam_embed = self.sam_image_encoder(sam_input)
        #将原始输入传递给编码器部分，得到编码器的跳跃连接结果skips
        skips = self.encoder(x)
        # 对sam_embed进行双线性插值，调整其大小以匹配skips[3]的大小。
        print(f'the shape of the sam_embedding is: {sam_embed.shape}')
        print(f'the shape of the Unet embedding is: {skips[3][].shape}')
        sam_embed = F.interpolate(sam_embed, size=(skips[3].shape[2], skips[3].shape[3]), mode='bilinear', align_corners=True)
        
        
        #将sam_embed与skips[3]进行concat
        skips[3] = torch.cat((skips[3], sam_embed), dim=1)

        #将拼接后的特征表示传递给解码器部分，得到模型的输出
        return self.decoder(skips)
